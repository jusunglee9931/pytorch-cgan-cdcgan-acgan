...
def train(model,trl,gd):
    ...
    for i in range(model.epoch):
     ...
     for m,(image,label) in enumerate(trl):
       ...
       one_hot_label = torch.FloatTensor(model.batch_size,10).zero_()
...
def generateimage(m,gd,e,new):
       ...
       for i in range(grid*grid):
        ...
        ax.set_axis_off()
        idx = int(i / lable_size) * m.batch_size + i % lable_size
...
g_gd = np.tile(g_gd, (lable_size,1))
g_gs = Variable(torch.from_numpy(g_gd).view(-1,g_arg.z_size).cuda())

g_c  = np.linspace(0,lable_size-1,lable_size).astype(np.int)
g_c  = np.repeat(g_c, g_arg.batch_size)
g_c = torch.LongTensor(g_c)

g_one_hot_label = torch.FloatTensor(lable_size*g_arg.batch_size,lable_size).zero_()
